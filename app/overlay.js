...
.panel{
  position:fixed; left:10px; top:10px; display:flex; align-items:center; gap:8px; flex-wrap:wrap;
...
.panel.compact{ gap:6px; }
.panel.collapsed .adv{ display:none; }

.panel input[type="text"]{width:90px;padding:6px 8px;border-radius:8px;border:1px solid #333;background:#0f0f12;color:#fff}
.panel .box{display:flex;align-items:center;gap:6px;background:#141416;padding:4px 6px;border-radius:8px;border:1px solid #2b2b2b}
.panel .box input{width:60px;padding:4px 6px;border-radius:6px;border:1px solid #333;background:#0f0f12;color:#fff}
...
.fab{
  position:fixed; left:10px; top:10px; width:36px; height:36px; border-radius:18px; background:#0b5cff; color:#fff;
  border:none; cursor:pointer; display:none; pointer-events:auto; font-weight:700; box-shadow:0 8px 24px rgba(0,0,0,.45)
}
...
.arrow{
  ...
  box-shadow:0 2px 10px rgba(0,0,0,.25);pointer-events:none;user-select:none;will-change: transform; left:0; top:0;
}
.arrow::after{
...
<div id="kbf-panel" class="panel">
  ...
  <button id="kbf-more" title="Mostrar/ocultar opções">Mais ▾</button>

  <div class="sep adv"></div>

  <button id="kbf-scope" class="adv">Escopo</button>
  <button id="kbf-unscope" class="adv">Desfazer</button>
  ...
  <div class="sep adv"></div>
  <button id="kbf-del" class="adv" title="Remover falsos positivos">Excluir</button>

  <div class="sep adv"></div>
  <button id="kbf-hide" class="adv" title="Fechar painel">✕</button>
</div>
`;
    ...
    const ui = {
      ...
      btnRescan: sh.getElementById("kbf-rescan"), // (opcional)
  ...
  function applyPanelVisibility(ui, visible) {
    if (!ui) return;
    ui.panel.style.display = visible ? "flex" : "none";
    ui.fab.style.display   = visible ? "none" : "block";
  }

  function renderCollapsed(ui, collapsed) {
    if (!ui) return;
    ui.panel.classList.toggle('collapsed', !!collapsed);
    if (ui.btnMore) ui.btnMore.textContent = collapsed ? "Mais ▾" : "Menos ▴";
  }

  function fitPanel(ui) {
    if (!ui) return;
    ui.panel.classList.toggle('compact', window.innerWidth < 560);
  }

  window.kbf.overlay = { mountOverlay, applyPanelVisibility, renderCollapsed, fitPanel };
})();
